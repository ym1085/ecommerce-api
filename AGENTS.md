...
- CRITICAL: 사용자가 "수정해"·"반영해"·"만들어줘"처럼 명시적으로 편집을 지시하기 전에는 파일을 생성·수정·삭제하지 않는다
...
- `CLAUDE.md`의 `12. 코드·로직 설명 형식`은 Claude 전용이며 Codex 응답에는 적용하지 않는다
- 로직 설명에는 메서드 시그니처와 호출 전후의 핵심 준비·분기·결과 코드를 포함해,
  해당 로직만 읽어도 흐름을 이해할 수 있는 범위의 실제 코드와 처리 흐름을 함께 제시한다
