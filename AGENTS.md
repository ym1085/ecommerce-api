...
- `//` 주석은 복잡한 분기·보안 판단·설계 결정에만 쓰고 Javadoc을 포함한 모든 주석 끝에 마침표를 붙이지 않는다
...
- 로직 설명에는 메서드 시그니처~닫는 중괄호를 기본 발췌 단위로 삼아 준비·분기·결과를 한 눈에 보이게 한다.
  변경 줄만 오려내지 않는다. 생략은 `...` 한 줄로 표기하되 조건·의존성·반환 처리는 생략하지 않는다
- 요청하지 않은 배경 설명·반복 요약을 덧붙이지 않는다
- 여러 단계나 해야 할 일을 안내할 때는 `1.`, `2.`, `3.` 형태의 번호 목록으로 순서를 구분한다
- `CLAUDE.md`의 `설명 형식` 섹션은 Claude 전용이며 Codex 응답에는 적용하지 않는다
...
- 서로 다른 의미 단위의 문단 사이에는 반드시 빈 줄 1줄을 둔다. 헤더·본문·코드 블록 사이에도 빈 줄 1줄을 두어 내용이 붙어 보이지 않게 한다
- 코드·로직 분석·설명·검토·제안·수정 보고에 적용하고, 단순 질의응답·한두 문장 답변에는 강제하지 않는다
- 맨 앞에 결론을 **굵게** 한 줄로 두괄식으로 쓴다. 제목만 읽고도 무엇을 어떻게 할지 판단되게 쓴다
- 이어 모든 제목을 # 고정 헤더로 나눠 본문보다 크게 하고 상황별로 이름을 바꾸지 않는다. 제목은 # 하나만 쓰고 본문은 일반 텍스트로 둔다. 수정·버그는 무엇이 문제인가 → 어떻게 고치나 → 남은 결정, 신규 구현은 설계 → 구현 → 남은 결정, 분석·리뷰는 근거만 둔다
- 인과·판단·근거는 문장으로 서술하고 - 불릿은 병렬 나열에만 쓴다. 짧은 불릿을 줄줄이 늘어놓지 않는다
- 수정 지점이 여러 개면 결론과 원인은 한 번만 쓰고 # 파일명:라인 — 요약 아래에 지점별 코드를 반복한다. 지점이 5개를 넘으면 표로 목록을 먼저 제시하고 코드를 잇는다
- 위치는 항상 파일명:라인으로 명시하고 "여기"·"이 부분" 같은 표현은 쓰지 않는다
- 변경은 +·-가 있는 diff 블록, 신규·최종 코드는 해당 파일 언어의 코드 블록을 쓴다. 발췌는 메서드 시그니처~닫는 중괄호를 기본 단위로 삼아 준비·분기·결과가 한 눈에 들어오게 한다. 생략은 ... 한 줄로 표기하되 이해에 필요한 조건·의존성·반환 처리는 생략하지 않는다
- 코드 설명 시 import 문은 보여주지 않는다. 변경·추가된 로직만 보여준다
- 서브에이전트 결과는 그대로 옮기지 않고 이 형식으로 다시 쓴다. 결과가 충돌하면 채택한 쪽과 근거를 남은 결정에 남긴다
- 구분선(---)을 쓰지 않고, 서론·배경·반복 요약·미사여구·HTML 태그를 쓰지 않는다. 번호 제목도 원칙적으로 쓰지 않되 아래 CASE 규칙은 예외로 둔다
- 고정 섹션 제목에는 다음 이모지를 접두어로 붙인다: 무엇이 문제인가 🚨, 어떻게 고치나 🛠️, 남은 결정 ❓, 설계 📐, 구현 ⚙️, 근거 🔍. `# CASE`처럼 번호가 붙는 제목에는 이모지를 쓰지 않는다
- 문제·수정 지점이 2개 이상인 보고는 각 제목을 `# CASE 001`, `# CASE 002`로 번호를 달고, "어떻게 고치나"에서도 같은 CASE 번호를 제목에 달아 1:1로 매칭한다. 목록 표를 먼저 줄 때 첫 열도 CASE 번호로 둔다. 지점이 1개면 번호를 붙이지 않는다
