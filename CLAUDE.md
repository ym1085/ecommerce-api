...
- 주석은 부족한 것보다 넉넉한 편이 낫다. 불필요하면 사용자가 직접 걷어내므로, 기본값은 "충분히 설명".
- 스타일은 기존 코드 컨벤션을 따른다. 메서드/클래스는 한글 Javadoc, 라인 설명은 `//`.
- Javadoc은 한 줄짜리라도 여러 줄 형식(`/**` 줄바꿈 후 `*` 본문, 줄바꿈 `*/`)으로 작성한다. 한 줄 `/** ... */`는 쓰지 않는다.
- 주석 끝에 마침표(`.`)를 붙이지 않는다.
- 클래스/공개 메서드에는 "무엇을/왜/주의점"이 드러나는 Javadoc을 붙인다 (파라미터·반환·설계 의도 포함).
- 이름만으로 자명한 단순 위임/게터성 코드까지 억지로 달지는 않되, 애매하면 다는 쪽으로 판단한다.
