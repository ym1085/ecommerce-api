...
> 아래 규칙은 이 절의 이전 규칙보다 우선한다.

- 서로 다른 의미 단위의 문단 사이에는 반드시 빈 줄 1줄을 둔다. 헤더·본문·코드 블록 사이에도 빈 줄 1줄을 둔다
- 코드·로직 분석·설명·검토·제안·수정 보고에 적용하고, 단순 질의응답·한두 문장 답변에는 강제하지 않는다
- 맨 앞에 결론을 **굵게** 한 줄로 두괄식으로 쓴다. 제목만 읽고도 무엇을 어떻게 할지 판단되게 쓴다
- 이어 모든 제목을 # 고정 헤더로 나눈다. 수정·버그는 무엇이 문제인가 → 어떻게 고치나 → 남은 결정, 신규 구현은 설계 → 구현 → 남은 결정, 분석·리뷰는 근거만 둔다
...
- 문제·수정 지점이 2개 이상인 보고는 각 제목을 `# CASE 1`, `# CASE 2`로 번호를 달고, "어떻게 고치나"에서도 같은 CASE 번호를 제목에 달아 1:1로 매칭한다. 목록 표를 먼저 줄 때 첫 열도 CASE 번호로 둔다. 지점이 1개면 번호를 붙이지 않는다
...
- 번호 제목·구분선(`---`)을 쓰지 않고, 서론·배경·반복 요약·미사여구·HTML 태그를 쓰지 않는다
